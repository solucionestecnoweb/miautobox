...
odoo.define('pos_stock.models',function(require) {
    ...
    models.load_fields('product.product',['qty_available','virtual_available','outgoing_qty','type','bom_count', 'is_manufacturing']);
	models.load_models([{
    ...
    models.Order = models.Order.extend({
        add_product: function(product, options){
            ...
            if(!self.pos.config.wk_continous_sale && self.pos.config.wk_display_stock && !self.pos.get_order().is_return_order) {
                ...
                if(qty_count <= self.pos.config.wk_deny_val && !product.is_manufacturing)
    ...
    models.Orderline = models.Orderline.extend({
        ...
        set_quantity: function(quantity, keep_price){
            ...
            if(self.stock_location_id && quantity && quantity!='remove'){
                if(self.pos.get_order() &&  self.pos.get_order().selected_orderline &&  self.pos.get_order().selected_orderline.cid == self.cid && !self.product.is_manufacturing){
                    Gui.showPopup('OutOfStockMessagePopup',{
                        'title':  _t("Warning !!!!"),
            ...
            // -------code for POS Warehouse Management----------------
            if((!self.pos.config.wk_continous_sale && self.pos.config.wk_display_stock && isNaN(quantity)!=true && quantity!='' && parseFloat(self.wk_line_stock_qty)-parseFloat(quantity)<self.pos.config.wk_deny_val && self.wk_line_stock_qty !=0.0 ) && !this.option.product.is_manufacturing){
                Gui.showPopup('OutOfStockMessagePopup',{
                    'title':  _t("Warning !!!!"),
                ...
                if (self.pos.get('selectedOrder')) {
                    ...
                    if (!self.pos.config.wk_continous_sale && self.pos.config.wk_display_stock && wk_pro_order_line) {
                        ...
                        if (wk_avail_pro < self.pos.config.wk_deny_val && (!(quantity == '' || quantity == 'remove')) && !wk_pro_order_line.product.is_manufacturing) {
                            Gui.showPopup('OutOfStockMessagePopup',{
                                'title':  _t("Warning !!!!"),
