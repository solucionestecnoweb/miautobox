odoo.define('mt_pos_payment.screens', function (require) {
    ...
    models.load_fields('pos.payment.method', 'calculate_wh_itf');
    const ReceiptScreen = (AbstractReceiptScreen) => {
        class ReceiptScreen extends AbstractReceiptScreen {
            ...
            async ImprimirFiscal() {
                ...
                order.get_paymentlines().forEach((payment_lines) => {
                    payment_order_lines.push({
                            'name': payment_lines.name,
                            'payment_method':  payment_lines.payment_method.name,
                            'amount': payment_lines.amount,
                            'calculate_wh_itf': payment_lines.payment_method.calculate_wh_itf
                    })   
                })
                ...
                window.open("https://desarrollo.sistemas24.com/cuadro/facturacion_fiscal/test.php?cid=" + order.cid +"&numero_recibo=" + order.name + "&cliente=" + order.changed.client.name + "&telefono=" + order.changed.client.phone + "&direccion=" + order.changed.client.address + "&rif_cedula=" + order.changed.client.vat + "&lineas=" + enviar_lineas+ "&payment_order_lines=" + line_payments +"&order_id="+666,"_blank", "width=786, height=786")
