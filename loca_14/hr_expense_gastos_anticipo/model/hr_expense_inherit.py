...
class HrExpenseSheet(models.Model):
    ...
    state_asiento = fields.Char(compute='_compute_state_asiento')
    asiento_gastos_viejo_id = fields.Many2one('account.move')

    def draft(self):

        self.state="draft" #"approve"
        self.asiento_gastos_viejo_id=self.account_move_id.id
        #self.account_move_id.filtered(lambda move: move.state == 'posted').button_draft()
        #self.account_move_id.with_context(force_delete=True).unlink()
        self.account_move_id.state='cancel'
        """busca=self.env['account.move'].search([('id','!=',self.account_move_id.id),('state','=','cancel')])
        #raise UserError(_('busca: %s')%busca)
        if busca:
            for det in busca:
                det.filtered(lambda move: move.state == 'posted').button_draft()
                det.with_context(force_delete=True).unlink()"""
                
        if self.payment_ids:
            for rec in self.payment_ids:
                rec.saldo_disponible=rec.amount
                rec.usado=False
        self.status_saldo_pendiente='earring'

    def _compute_state_asiento(self):
        valor='NULL'
        if self.account_move_id:
            valor=self.account_move_id.state
        self.state_asiento=valor
    ...
    def _compute_dif_pendiente(self):
        ...
        for anti in self.payment_ids:
            if anti.currency_id.id==self.env.company.currency_id.id:
                acom=acom+anti.saldo_disponible#anti.amount_signed  MONTO EN BS
                acom2=acom2+(anti.saldo_disponible/anti.rate)  # MONTO EN $
            else:
                acom=acom+anti.saldo_disponible*anti.rate  # MONTO EN BS
                acom2=acom2+anti.saldo_disponible  # MONTO EN $
        resultado=(acom-self.total_amount)
        resultado2=(acom2-self.total_amount_signed)
    ...
    def action_sheet_move_create(self): # aqui publica
        ...
        self.cotea_anticipo()
        self.asigna_asiento_anticipo_diferencia()

    def asigna_asiento_anticipo_diferencia(self):
        """xfind = self.env['account.gasto.anticipo'].search([
            ('employee_id', '=', self.employee_id.id),
            ('anticipo', '=', True),
            ('state', '=', 'posted'),
        ...
        # PARA INCORPORAR LINEAS DE ASIENTOS
        self.account_move_id.state='draft'
        sig=1
        cuenta_id = self.env.company.account_anti_employee_payable_id.id
        # INCORPORO LINEAS DE APUNTE CONTABLE DE EXISTIR ANTICIPO
        if self.payment_ids:
            for rec in self.payment_ids:
                if rec.currency_id!=self.company_id.currency_id:
                    total_amount=-1*(rec.amount-rec.saldo_disponible)*rec.rate
                    total_amount_currency=-1*(rec.amount-rec.saldo_disponible)
                else:
                    total_amount=-1*(rec.amount-rec.saldo_disponible)
                    #total_amount_currency=-1*(rec.amount-rec.saldo_disponible)/rec.rate
                move_line_dst = {
                    'name': "Anticipo a "+self.employee_id.name,
                    'debit': total_amount > 0 and total_amount,
                    'credit': total_amount < 0 and -total_amount,
                    'account_id': cuenta_id,
                    'date_maturity': rec.payment_date,
                    'amount_currency': total_amount_currency if rec.currency_id!=self.company_id.currency_id else 0.0,
                    'currency_id': rec.currency_id.id if rec.currency_id!=self.company_id.currency_id else False,
                    #'expense_id': self.id,
                    'partner_id': self.employee_id.address_home_id.commercial_partner_id.id,
                    #'asiento_gastos':True,
                    #'tasa_gastos':rec.rate,
                    'move_id':self.account_move_id.id,
                }
                move_line_obj = self.env['account.move.line']
                move_line_id1 = move_line_obj.create(move_line_dst)
                #move_line_values.append(move_line_dst)
        # INCORPORO LINEAS DE APUNTE CONTABLE DE EXISTIR diferencias de gastos
        if self.monto_diferencia:
            if self.monto_diferencia_uds==0:
                self.monto_diferencia_uds=1
            rate=self.monto_diferencia/self.monto_diferencia_uds
            move_line_dif_empl = {
            'name': "DIFERENCIA DE GASTOS A PAGAR A EMPLEADO"+self.employee_id.name,
            'debit':0.0,
            'credit':self.monto_diferencia,
            ...
            move_line_obj = self.env['account.move.line']
            move_line_id2 = move_line_obj.create(move_line_dif_empl)

        ### A PARTIR DE AQUI VERIFICA SI ES NECESARIO HACER UN ASIENTO DIFERENCIAL DE CAMBIO
        verifica_balance=0
        suma=self.env['account.move.line'].search([('move_id','=',self.account_move_id.id)])
        for suma_det in suma:
            verifica_balance=verifica_balance+suma_det.balance
        #raise UserError(_('busca: %s')%verifica_balance)
        if verifica_balance!=0:
            move_line_dif_cambiaria = {
            'name': "Diferencial Cambiario Emp: "+self.employee_id.name,
            'debit':abs(verifica_balance) if verifica_balance<0 else 0.0,
            'credit':abs(verifica_balance) if verifica_balance>0 else 0.0,
            'account_id':self.env.company.account_perd_dif_camb_id.id if verifica_balance>0 else self.env.company.account_gana_dif_camb_id.id,
            'date_maturity': self.accounting_date,
            #'amount_currency': total_amount_currency if different_currency else 0.0,
            #'currency_id': expense.currency_id.id if different_currency else False,
            #'expense_id': expense.id,
            ...
            move_line_obj = self.env['account.move.line']
            move_line_id3 = move_line_obj.create(move_line_dif_cambiaria)
        ### FIN A PARTIR DE AQUI VERIFICA SI ES NECESARIO HACER UN ASIENTO DIFERENCIAL DE CAMBIO
        
        self.account_move_id.post()
        busca=self.env['account.move'].search([('id','=',self.asiento_gastos_viejo_id.id),('state','=','cancel')])
        #raise UserError(_('busca: %s')%busca)
        if busca:
            for det in busca:
                det.filtered(lambda move: move.state == 'posted').button_draft()
    ...
    def action_submit_sheet(self):
        ...
        for rac in self.expense_line_ids:
            if rac.tasa_personalizada==True and rac.currency_id.id!=rac.company_currency_id.id:
                #raise UserError(_('hola: %s')%rac.rate)
                ...
                if det_lin:
                    for det_fac in det_lin:
                        ...
                        if self.state!="cancel":
                            tasa=(det_fac.total_amount_company/det_fac.total_amount)
                            base=base+det_fac.untaxed_amount*tasa
                            total=total+det_fac.total_amount*tasa
                            id_impuesto=det_fac.tax_ids.id
                            total_impuesto=total_impuesto+(det_fac.total_amount-det_fac.untaxed_amount)*tasa
                            if tipo_alicuota=="general":
                                alicuota_general=alicuota_general+(det_fac.total_amount-det_fac.untaxed_amount)*tasa
                                base_general=base_general+det_fac.untaxed_amount*tasa
                                valor_iva=det_fac.tax_ids.amount
                            if tipo_alicuota=="exempt":
                                total_exento=total_exento+det_fac.untaxed_amount*tasa
                            if tipo_alicuota=="reduced":
                                alicuota_reducida=alicuota_reducida+(det_fac.total_amount-det_fac.untaxed_amount)*tasa
                                base_reducida=base_reducida+det_fac.untaxed_amount*tasa
                            if tipo_alicuota=="additional":
                                alicuota_adicional=alicuota_adicional+(det_fac.total_amount-det_fac.untaxed_amount)*tasa
                                base_adicional=base_adicional+det_fac.untaxed_amount*tasa
                        ...
                        #'company_id':self.company_id.id,#loca14  OJO VALIDAR DESPUES
...
class AccountGastoAnticipo(models.Model):
    ...
    def registro_movimiento_linea(self,id_movv):
        ...
        cuenta_prove_pagar = self.journal_id.default_account_id.id #loca14
        ...
        value = {
             ...
             #'asiento_gastos':self.tasa_personalizada,
             #'tasa_gastos':self.rate,
        ...
        #value['asiento_gastos'] = self.tasa_personalizada
        #value['tasa_gastos'] = self.rate
...
class ResCompany(models.Model):
    ...
    account_gana_dif_camb_id = fields.Many2one('account.account',company_dependent=True)
    account_perd_dif_camb_id = fields.Many2one('account.account',company_dependent=True)
